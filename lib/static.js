...
const { getOptions, getAssetsDir, getPath, getStaticDir, getRevealOptions, getSlideOptions, getTemplate } = require('./config');
...
const copyFileDependencies = async (filePath, extraOptions) => {
  ...
  deps.filter(function(dep) {
        const srcpath = dep.src.replace("{{{base}}}/module",modulesBasePath)
