...
const { getOptions, getAssetsDir, getPath, getStaticDir, getRevealOptions, getSlideOptions, getTemplate, getFilesGlob } = require('./config');
const { isDirectory, isFile, parseYamlFrontMatter, getFilePaths } = require('./util');
const { revealBasePath, modulesBasePath, highlightThemePath } = require('./constants');
...
const copyAssetsAndWriteFile = async (sourceDir, file, targetDir) => {
  ...
  awaits.push(await copyFileDependencies(path.join(sourceDir, file), { base }));
...
const copyFileDependencies = async (filePath, extraOptions) => {
  const staticDir = getStaticDir();
  ...
  const deps = revealOptions.dependencies;
  var depdirs = [];
  deps.filter(function(dep) {
        const srcpath = dep.src.replace("module",modulesBasePath)
        const src = path.resolve(srcpath, '..');
        return depdirs.indexOf(src) >= 0 ? false : depdirs.push(src);
  })

  //TODO copy template deps
  //const template = await getTemplate(options.template);

  await Promise.all(
    depdirs.map(dep => {
      const srcdir = dep;
      const destdir = path.join(staticDir, 'module',relativeDir(modulesBasePath, srcdir))
      return cp(srcdir, destdir)
    }
    )
  );
  return;
